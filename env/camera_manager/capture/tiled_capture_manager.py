...
class TiledCaptureManager:
    ...
    def init_cameras(self):
        ...
        3. Attach Annotator
        """
        # CameraManager may replace these lists after a hard scene reset.
        # Always bind to its current handles before creating render products.
        self.cameras = self.camera_manager.cameras
        self.camera_names = self.camera_manager.camera_names
        self.num_cams = len(self.cameras[0])
    ...
    def destroy(self):
        ...
        """
        cameras = list(self.tiled_cameras)
        try:
            for tiled_camera in cameras:
                try:
                    tiled_camera.destroy()
                except Exception as exc:
                    # Hard reset replaces the whole stage.  One stale
                    # Replicator node must not prevent the remaining cameras,
                    # robot, scene and simulator from being released.
                    print(
                        "[camera cleanup] tiled camera teardown warning: "
                        f"{type(exc).__name__}: {exc}",
                        flush=True,
                    )
        finally:
