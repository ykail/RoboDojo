...
Optional:
  ...
  --target-episodes N        Right-accepted episodes to collect (default: 50).
...
if [[ -z "${DATASET_ID}" ]]; then
    ...
    DATASET_ID="robodojo_${TASK}_x5_online_dagger_${POLICY_NAME}_${DIGEST_HEX:0:12}_${EXPECTED_KAI0_COMMIT:0:8}_simstep25_v3"
fi
...
if [[ "${CAPTURE_MODE}" == "raw-deferred" ]]; then
    ...
    echo "[acOne Isaac] one Right = one complete policy+human LeRobot episode"
    echo "[acOne Isaac] timing=one simulator transition per 25Hz row; no fill frames"
    echo "[acOne Isaac] target=${TARGET_EPISODES}; resume uses committed episode count"
fi
