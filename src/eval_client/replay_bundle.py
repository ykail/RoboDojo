...
class ReplayFrame:
    ...
    source_policy_provenance: dict[str, Any]
    ...
    *,
    ...
    try:
        with np.load(state_path, allow_pickle=False) as archive:
            ...
            selected = (
                int(np.abs(timestamps - float(time_s)).argmin())
    ...
    raw_policy_provenance = metadata.get("robodojo_policy_provenance")
    source_policy_provenance = (
        dict(raw_policy_provenance)
        if isinstance(raw_policy_provenance, dict)
        else {}
    )
    return ReplayFrame(
        dataset_root=root,
        ...
        manifest=manifest,
        source_checkpoint=str(metadata.get("robodojo_checkpoint", "")),
